...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        #Validar se o usuário ja é um participante
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        draw.text((230, 706), f"{participante.username}", font=fonte_nome, fill=(0,0,0))
        draw.text((760, 805), f"{evento.nome}", font=fonte_info, fill=(0,0,0))
        draw.text((816, 874), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0,0,0))
